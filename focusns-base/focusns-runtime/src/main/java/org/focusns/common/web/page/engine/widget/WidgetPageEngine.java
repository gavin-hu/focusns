            ...
            for(String position : pageConfig.getPositionConfigMap().keySet()) {
                ...
                for(WidgetConfig widgetConfig : positionConfig.getWidgetConfigMap().values()) {
                   ...
                   /* if(StringUtils.hasText(widgetConfig.getValidationTarget())) {
                        ...
                        //
                        Class<?> targetClass = ClassUtils.forName(widgetConfig.getValidationTarget());
                        Class<?>[] groupClasses = new Class[]{};
                        if(!widgetConfig.getValidationGroups().isEmpty()) {
                            List<Class<?>> groupClassList = new ArrayList<Class<?>>();
                            for(String validationGroup : widgetConfig.getValidationGroups()) {
                                groupClassList.add(ClassUtils.forName(validationGroup));
                            }
                            groupClassList.add(Default.class);
                            groupClasses = groupClassList.toArray(new Class[groupClassList.size()]);
                        }
                        ValidatorFactory validatorFactory = getApplicationContext().getBean(ValidatorFactory.class);
                        ValidatedBean validatedBean = ValidationHelper.createForClass(validatorFactory, targetClass, groupClasses);
                        context.put("validatedBean", validatedBean);
                        //
                        context.put("widgetConfig", widgetConfig);
                        String validationDir = getServletContext().getRealPath("/WEB-INF/generate/validation");
                        OutputStream output = new FileOutputStream(new File(validationDir, widgetConfig.getValidationTarget()+".js"));
                        //
